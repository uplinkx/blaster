...
void	pause_menu_init(t_pmenu *menu, void *pause_addr, void *pbackground, void *context, void *restart_fn)
{
	...
	SDLX_Button_Init(&(menu->redo), fetch_pause_sprite, MREDO_NORM, (SDL_Rect){(PLAY_WIDTH - 64) / 2, 270, 64, 64}, NULL);
	...
	SDLX_Button_Init(&(menu->menu), fetch_pause_sprite, MMENU_NORM, (SDL_Rect){MENU_LOFFSET, 270, 64, 64}, NULL);
