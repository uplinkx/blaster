...
SDL_bool	faser_fire(SDL_UNUSED t_weapon *weapon)
{
	SDL_bool	result;
	SDL_Rect	play_area;
...
void	faser_update(void *self, SDL_UNUSED void *meta)
{
	t_bullet	*bullet;
	SDL_Rect	play_area;
	static int	missed;
...
void	faser_factory(t_bullet *dst, SDL_UNUSED SDL_Point spawn_point, SDL_UNUSED double angle, SDL_UNUSED void *meta)
{
...
{
	t_weapon	faser_cannon;

	faser_cannon.type = B_MAINHAND;
	SDLX_new_Sprite(&(faser_cannon.ability_icon));
	fetch_faser_sprite(&(faser_cannon.ability_icon.sprite_data), 0);

	SDLX_new_Sprite(&(faser_cannon.treasure_sprite));
	fetch_faser_sprite(&(faser_cannon.treasure_sprite.sprite_data), 1);

	faser_cannon.start = 0;
	faser_cannon.cooldown = FASER_COOLDOWN;

	faser_cannon.curr = FASER_COOLDOWN;

	faser_cannon.isEnabled = SDL_TRUE;
	faser_cannon.meta_int = 0;
